...
use super::{
    map::FxHashMap,
    parser::{ParseCursor, ParseSequence},
};
...
impl Intern {
    pub fn new(str: impl fmt::Display) -> Self {
        thread_local! {
            static GLOBAL_INTERNER: (StrongEntity, Obj<Interner>) = {
        ...
        GLOBAL_INTERNER.with(|(_, interner)| interner.intern(str))
    }
...
macro_rules! intern {
    ($ini:expr) => {{
        $crate::util::span::intern_macro_internals::thread_local! {
            static __INTERN: $crate::util::span::intern_macro_internals::Intern =
                $crate::util::span::intern_macro_internals::Intern::new($ini);
        }
...
impl FileData {
    pub fn root_loc(&self) -> FileLoc {
        FileLoc {
            file: self.me,
            pos: FilePos { line: 0, column: 0 },
        }
    }
}
